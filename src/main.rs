...
fn prepare_request<B>(mut req: Request<B>, client_addr: SocketAddr) -> Result<Request<B>> {
...
async fn proxy(req: Request<Incoming>, client_addr: SocketAddr) -> Result<Response<Incoming>> {
    let prepped_req = prepare_request(req, client_addr)?;
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(e) = server_http1::Builder::new()
                .serve_connection(io, service_fn(|req| proxy(req, client_addr)))
                .await
            {
...
mod tests {
    ...
    fn prepare_request_sets_xff_when_absent() -> Result<()> {
        ...
        let prepped_req = prepare_request(orig_req, CLIENT_ADDR)?;
    ...
    fn prepare_request_rewrites_host_to_backend() -> Result<()> {
        ...
        assert_eq!(
            prepped_req.headers().get(HOST).map(HeaderValue::as_bytes),
