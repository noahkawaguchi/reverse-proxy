...
pub(crate) struct BackendGuard {
    backend: Arc<Backend>,
}

impl BackendGuard {
    pub(crate) fn addr(&self) -> SocketAddr { self.backend.addr() }
}
...
impl Backend {
    ...
    pub(crate) const fn healthy(addr: SocketAddr) -> Self {
    ...
    #[cfg(test)]
    pub(crate) const fn unhealthy(addr: SocketAddr) -> Self {
    ...
    pub(crate) fn acquire(self: Arc<Self>) -> BackendGuard {
        self.connections.fetch_add(1, Ordering::Relaxed);
        BackendGuard { backend: self }
    }

    pub(crate) const fn addr(&self) -> SocketAddr { self.addr }

    /// Atomically retrieves the health status of the `Backend`.
    pub(crate) fn is_healthy(&self) -> bool { self.healthy.load(Ordering::Relaxed) }

    /// Atomically sets the health status of the `Backend`.
    pub(crate) fn set_health(&self, new_health: bool) {
        self.healthy.store(new_health, Ordering::Relaxed);
    }

    /// Atomically retrieves the number of active connections to the `Backend`.
    pub(crate) fn num_connections(&self) -> usize { self.connections.load(Ordering::Relaxed) }
}
